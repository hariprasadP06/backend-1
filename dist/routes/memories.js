...
const session_middleware_js_1 = require("./middleware/session-middleware.js");
const memoryApi = new hono_1.Hono();
...
memoryApi.post('/', session_middleware_js_1.sessionMiddleware, async (c) => {
    try {
        ...
        const memory = await index_js_1.prismaClient.memory.create({
            data: {
...
memoryApi.get('/', session_middleware_js_1.sessionMiddleware, async (c) => {
    try {
        const user = c.get("user");
        if (!user) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        if (isNaN(limit) || limit < 1)
            limit = 10;
        const skip = (page - 1) * limit;
        const memories = await index_js_1.prismaClient.memory.findMany({
            where: {
                userId: user.id,
                ...(tag && { tags: { has: tag } }),
            },
...
memoryApi.get('/:id', session_middleware_js_1.sessionMiddleware, async (c) => {
    try {
        const user = c.get("user");
        if (!user) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        const id = c.req.param('id');
        const memory = await index_js_1.prismaClient.memory.findFirst({
            where: {
                id,
                userId: user.id,
            },
...
memoryApi.put('/:id', session_middleware_js_1.sessionMiddleware, async (c) => {
    try {
        const user = c.get("user");
        if (!user) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        const existing = await index_js_1.prismaClient.memory.findFirst({
            where: { id, userId: user.id },
        });
        if (!existing)
            return c.json({ error: 'Memory not found or unauthorized' }, 404);
        const updated = await index_js_1.prismaClient.memory.update({
            where: { id },
            data: { title, content, tags },
        });
...
memoryApi.delete('/:id', session_middleware_js_1.sessionMiddleware, async (c) => {
    try {
        const user = c.get("user");
        if (!user) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        if (!existing)
            return c.json({ error: 'Memory not found or unauthorized' }, 404);
        await index_js_1.prismaClient.memory.delete({ where: { id } });
        return c.json({ message: 'Memory deleted' });
    }
    catch (error) {
